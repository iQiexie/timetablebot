...
class ClassesRepo(BaseRepo[ClassModel]):
    ...
    async def update_classes(self, classes: List[ScraperResult]) -> None:
        ...
        for arg_set in query_args_sets:
            unique_arg_sets = set()
            deduplicated_arg_sets = []

            for i in arg_set:
                i_str = f"{i['value']}_{i['group_number']}_{i['week_day']}_{i['line_position']}_{i['duration']}"

                if not i_str in unique_arg_sets:
                    unique_arg_sets.add(i_str)
                    deduplicated_arg_sets.append(i)

            stmt = insert(self.model).values(deduplicated_arg_sets)
