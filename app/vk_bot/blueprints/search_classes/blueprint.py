...
async def pattern_search(message: Message):
    greeting = (
        ...
        "которые соответствуют твоему ключу (шаблону) \n\n"
        "Например, ты можешь найти расписание преподавателя, написав его имя, а бот покажет его "
        "расписание на эту и следующую неделю. Можешь написать только часть имени, но тогда "
        "поиск будет менее точным"
    )

    await message.answer(greeting)
    await message.answer(
        'Напиши мне свой запрос, а потом выбери нужный день'
    )
