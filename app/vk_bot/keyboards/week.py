...
def compose_detailed_menu(pattern: str = None) -> Keyboard:
    ...
    keyboard.add(
        Text("👈 Эта неделя", {"cmd": "sweek", "next": False, "match": pattern}), KeyboardButtonColor.SECONDARY,
    )
    keyboard.add(
        Text("👉 Следующая неделя", {"cmd": "sweek", "next": True, "match": pattern}), KeyboardButtonColor.SECONDARY,
    )

    if pattern:
        keyboard.row()
        keyboard.add(
            Text("⚠️ Режим поиска по шаблону", {"cmd": "searching_status"})
        )

    else:
        keyboard.row()
        keyboard.add(
            Text("👩‍🏫 Поиск по преподавателю", {"cmd": "pattern_search"}), KeyboardButtonColor.SECONDARY
        )
...
def compose_week_keyboard(next_week: bool, pattern: str = None):
    ...
    keyboard.add(
        Text("Назад", payload={"cmd": "detailed", 'match': pattern}), color=KeyboardButtonColor.NEGATIVE
    )
...
reset_keyboard.add(Text("Сбросить режим поиска по шаблону", payload={"cmd": "detailed"}), color=KeyboardButtonColor.NEGATIVE)